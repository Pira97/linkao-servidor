...
public static class Chat
{
    ...
    private static bool HandleCommand(int userIndex, string chat)
    {
        ...
        switch (cmd)
        {
            ...
            case "/poderdioses": case "/poderdioseson": return Cmd_PoderDiosesIniciar(u, parts);
            case "/poderdiosesoff": return Cmd_PoderDiosesFinalizar(u);
            case "/estadopoderdioses": return Send(u, PoderDioses.Estado());
            ...
            case "/smsg": case "/smessage": return Cmd_SMsg(u, parts);
            case "/rmsg": case "/rmessage": return Cmd_RMsg(u, parts);
    ...
    /// <summary>/PODERDIOSES [minutos] — GM (Dios): vuelve a prender el Poder de los Dioses (sin minutos = sin límite).
    /// Normalmente no hace falta: arranca prendido con el server (Dat/PoderDioses.ini Siempre=1).</summary>
    private static bool Cmd_PoderDiosesIniciar(User u, string[] parts)
    {
        if (u.FaccionStatus < AdminLoader.STATUS_DIOS) return Send(u, "Solo los dioses pueden activar el Poder de los Dioses.");
        if (PoderDioses.EventoActivo) return Send(u, "El evento poder de dioses ya está activo. " + PoderDioses.Estado());
        int minutos = parts.Length >= 2 && int.TryParse(parts[1], out int m) ? m : 0;
        PoderDioses.Iniciar(u.Name, minutos);
        return Send(u, PoderDioses.Estado());
    }

    /// <summary>/PODERDIOSESOFF — GM (Dios): apaga el Poder de los Dioses hasta /PODERDIOSES o el próximo reinicio.</summary>
    private static bool Cmd_PoderDiosesFinalizar(User u)
    {
        if (u.FaccionStatus < AdminLoader.STATUS_DIOS) return Send(u, "Solo los dioses pueden finalizar el Poder de los Dioses.");
        if (!PoderDioses.EventoActivo) return Send(u, "El evento poder de dioses está desactivado");
        PoderDioses.Finalizar();
        return true;
    }
    ...
    {
        if (u.FaccionStatus < AdminLoader.STATUS_CONSEJERO) return Send(u, "No tienes privilegios para usar este comando.");
        ...
        // Solo a otros GMs conectados (COMANDOS_SERVIDOR.txt: "Mensaje a todos los GMs conectados"),
        // no a todos los usuarios logueados.
        foreach (var other in UserListManager.UserList.Where(x => x?.Conn != null && x.flags.UserLogged && x.FaccionStatus >= AdminLoader.STATUS_CONSEJERO))
            ServerPackets.ConsoleMsg(other.Conn, $"[GM {u.Name}]: {msg}", 59);
        return true;
    }

    private static bool Cmd_SMsg(User u, string[] parts)
    {
        if (u.FaccionStatus < AdminLoader.STATUS_CONSEJERO) return Send(u, "No tienes privilegios para usar este comando.");
    ...
    private static bool Cmd_RMsg(User u, string[] parts)
    {
        if (u.FaccionStatus < AdminLoader.STATUS_CONSEJERO) return Send(u, "No tienes privilegios para usar este comando.");
        if (parts.Length < 2) return false;
        string msg = string.Join(" ", parts.Skip(1));
