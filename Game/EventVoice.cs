...
public static class EventVoice
{
    ...
    /// <summary>bit1: el anuncio es del Poder de los Dioses. El cliente deja apagar SU cartel en
    /// Opciones sin tocar el de los GMs (ver event_voice_ui.js). Un cliente viejo sólo mira el bit0.
    /// Sólo lo puede poner el server (AnunciarDelSistema); Anunciar lo filtra.</summary>
    public const byte FLAG_PODER_DIOSES = 2;
    ...
    /// <summary>
    /// Anuncio que dispara el PROPIO server (un evento automático, no un GM): sin chequeo de
    /// privilegios ni cooldown por GM — el que llama se encarga de no ametrallar. Mismo camino
    /// que Anunciar desde la cola en adelante (GmUserIndex 0 = nadie a quien confirmarle).
    /// </summary>
    public static void AnunciarDelSistema(string texto, byte voz, int mapa = 0, byte volumen = 100,
                                          byte flags = FLAG_MOSTRAR_TEXTO)
    {
        AsegurarConfig();
        texto = Limpiar(texto);
        ...
        voz = voz < Voces.Length ? voz : (byte)0;
        ArrancarWorker();
        if (!_cola.Writer.TryWrite(new Pedido(texto, Math.Max(0, mapa), voz, Math.Clamp(volumen, (byte)0, (byte)100),
                                              (byte)(flags & (FLAG_MOSTRAR_TEXTO | FLAG_PODER_DIOSES)), "", 0)))
            Console.WriteLine($"[EventVoice] cola llena, se descarta el anuncio del sistema: \"{texto}\"");
    }
    ...
    private static void Difundir(Pedido p, string url)
    {
        ...
        var gm = p.GmUserIndex > 0 ? UserListManager.UserList[p.GmUserIndex] : null;
