...
public static class GuildManager
{
    ...
    {
        if (u == null) return "";
        // Tag personal de la poción de Tag (SubTipo 19): va DESPUÉS del de clan. El cliente parte
        // el nombre por el primer '<', así que el nombre plano se sigue leyendo igual con los dos.
        string personal = string.IsNullOrWhiteSpace(u.TagPersonal) ? "" : " <" + u.TagPersonal.Trim() + ">";
        if (u.GuildIndex <= 0) return personal;
        var g = GetByNumber(u.GuildIndex);
        return (g != null ? " <" + g.Name + ">" : "") + personal;
    }
    ...
    private static void QuitarGema(User u, short gema)
    {
        for (int i = 1; i <= Constants.MAX_INVENTORY_SLOTS; i++)
        {
            if (u.Invent.Object[i].ObjIndex == gema && u.Invent.Object[i].Amount > 0)
            {
                ...
                if (u.Conn != null) ServerPackets.ChangeInventorySlot(u.Conn, u, (byte)i);
                return;
            }
        }
    }
