...
public static class Inventory
{
    ...
    public static bool PuedeUsarObjeto(User u, in ObjData.Obj od, out string motivo, bool validarNivel = true)
    {
        ...
        // Las monturas NO se gatean por nivel (VB6: por skill Equitacion en DoEquita). Las barcas SÍ: DoNavega
        // llama con validarNivel=true y además pide la skill de Navegación.
    ...
    public static void EquipItem(int userIndex, byte slot)
    {
        ...
        // Requisitos de uso (clase/raza/nivel/sexo). Barcos/Monturas se validan dentro de
        // DoNavega (nivel + Navegación) y DoEquita (Equitación).
    ...
    private static void DoNavega(User u, ref UserObj item, byte slot, ObjData.Obj od)
    {
        ...
        {
            // Nivel Y skill (23-sep-2026, "barcas comunes al nivel 25 se pueden usar con skills y las barcas
            // de shop y galeones al nivel 40"): MinELV del obj.dat (25 barcas/galeras, 40 galeones y los
            // del shop) además del MinSkill en Navegación de siempre (PuedeUsarSkill, Trabajo.bas).
            const int SK_NAVEGACION = 26; // eSkill.Navegacion
            bool esGm = u.FaccionStatus >= AdminLoader.STATUS_CONSEJERO;
            if (!esGm && !PuedeUsarObjeto(u, od, out string motivoNav))
