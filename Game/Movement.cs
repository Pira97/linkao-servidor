...
public static class Movement
{
    ...
    public static void MoveUserChar(int userIndex, byte nHeading)
    {
        ...
        {
            puedeMover = false;
            ServerPackets.ConsoleMsg(u.Conn, MapasPorNivel.MotivoRechazo(exit.Value.DestMap, u), 1);
        }
        ...
        if (puedeMover)
        {
            ...
            // Rastro de un solo tile para la compensación de lag del cuerpo a cuerpo: el
            // atacante ve a los demás con ~0,65 tiles de atraso (el colchón de moveQueue, ver
            // FASE_OBJETIVO_NET en game.html) más la subida del paquete. Guardando de dónde
            // vino y cuándo, el golpe puede seguir alcanzando al que acaba de salir del tile.
            u.PosPrev = u.Pos;
            u.PosPrevMs = Environment.TickCount64;
    ...
    public static void WarpUser(int userIndex, short destMap, short destX, short destY, bool fx = true, bool sonido = true)
    {
        ...
        var u = UserListManager.UserList[userIndex];
        // Un teleport no deja rastro: sin esto, el tile de origen seguiría siendo golpeable
        // durante la ventana de compensación de lag del melee (ver Combat.UsuarioAtaca), o sea
        // un golpe fantasma a alguien que ya está en otro mapa.
        u.PosPrevMs = 0;
