...
public static class NpcManager
{
    ...
    public static int FixesSelfTest()
    {
        ...
        Console.WriteLine("=== FIX4: reacción inmediata a un atacante nuevo ===");
        {
            ...
            // [[aggro pegajoso]] userB le pega: el NPC NO tiene que soltar a userA, así que este fix
            // tampoco tiene que girarlo hacia userB (antes sí lo hacía: era el bug reportado).
            int prevTarget = n.TargetUser;
            var uB = UserListManager.UserList[userB];
            ProvocarNpc(n, uB);
            Assert(n.TargetUser == userA, $"un atacante nuevo NO le roba el objetivo mientras el anterior sea perseguible (target={n.TargetUser})");
            ReaccionInmediataANuevoAtacante(n, uB, prevTarget);
            Assert(n.Heading == 9, "si se quedó con su objetivo, no gira hacia el atacante nuevo");

            // Pero si el objetivo anterior ya no se puede perseguir (acá: muere), el atacante nuevo
            // sí pasa a ser el objetivo, y la reacción inmediata vuelve a actuar.
            UserListManager.UserList[userA].flags.Muerto = 1;
            prevTarget = n.TargetUser;
            ProvocarNpc(n, uB);
            Assert(n.TargetUser == userB, $"con el objetivo anterior fuera de juego sí toma al atacante nuevo (target={n.TargetUser})");
            ReaccionInmediataANuevoAtacante(n, uB, prevTarget);
            Assert(n.Heading == H_E, $"y gira a encararlo de inmediato, que está al Este (heading={n.Heading})");
            UserListManager.UserList[userA].flags.Muerto = 0;
    ...
    /// <summary>
    /// [[aggro pegajoso]] ¿El NPC sigue trabado en combate con el objetivo que ya tenía? Mismo
    /// criterio que ObjetivoDeCriatura pero SIN efectos: no suelta el target ni toca nada, solo
    /// responde. Se usa para decidir si un atacante NUEVO puede robarle el objetivo (ProvocarNpc,
    /// GuardiasAI): si este método dice que sí, el nuevo golpe NO cambia a quién persigue.
    /// Un guardia además pierde el enganche si su objetivo dejó de ser enemigo de su ciudad
    /// (cambio de facción), que es lo que GuardiasAI ya valida en su propio paso de persecución.
    /// </summary>
    private static bool SigueTrabadoConSuObjetivo(NpcInstance n)
    {
        int idx = n.TargetUser;
        if (idx <= 0 || idx > UserListManager.LastUser) return false;
        var u = UserListManager.UserList[idx];
        if (u == null || !u.flags.UserLogged || u.flags.Muerto != 0 || u.Pos.Map != n.Map) return false;
        if (EsGmIntocable(u) || !NpcVeUsuario(n, u)) return false;
        if (n.NpcType == NPCTYPE_GUARDIASCITY && !EsEnemigoUsuario(n.Ciudad, u)) return false;
        return Math.Abs(u.Pos.X - n.X) <= CRIATURA_PERSECUCION_X
            && Math.Abs(u.Pos.Y - n.Y) <= CRIATURA_PERSECUCION_Y;
    }
    ...
    /// Facciones.SonarKillstreak para PvP: escala en Sounds.DeRacha).</summary>
    internal static void SonarKillstreakBot(NpcInstance atacante)
    {
        atacante.KillStreak++;
        // BotFaccion: 1=Armada, 2=Milicia, 3=Caos; sin facción, la blanca de Renegado.
        short part = atacante.BotFaccion switch
        {
            1 => Combat.PART_KILL_ARMADA,
            2 => Combat.PART_KILL_MILICIA,
            3 => Combat.PART_KILL_CAOS,
            _ => Combat.PART_KILL_RENEGADO,
        };
        Combat.EfectoKill(atacante.Map, (short)atacante.CharIndex, part);
        short snd = Sounds.DeRacha(atacante.KillStreak);
        if (snd == 0) return;
    ...
    private static void GuardiasAI(int map, NpcInstance n)
    {
        ...
        // [[aggro pegajoso]] Igual que CriaturaAI: si su objetivo está al lado, le pega a ÉL aunque
        // haya otro adyacente; y si el que está al lado es otro, le pega (no pasa de largo al que lo
        // tapa) pero sin robarle el target al objetivo con el que sigue trabado. Sin esto, el fix de
        // ProvocarNpc no alcanzaba: al tick siguiente el guardia cambiaba igual de objetivo.
        bool trabado = SigueTrabadoConSuObjetivo(n);
        if (trabado)
        {
            var uT = UserListManager.UserList[n.TargetUser];
            if (Math.Abs(uT.Pos.X - n.X) + Math.Abs(uT.Pos.Y - n.Y) == 1)
            { adjU = n.TargetUser; hU = HeadingTo(n.X, n.Y, uT.Pos.X, uT.Pos.Y); }
        }
        if (adjU > 0)
        {
    ...
    public static void InitBot(NpcInstance bot, int owner, string nick, byte heading = 0)
    {
        ...
        // El primer CharacterCreate (SpawnAt → AreaVisibility.OnNpcSpawn, ANTES de este InitBot) salió
        // con el nombre genérico "Bot <clase>" de la definición cacheada en NpcData — recién ACÁ se
        // conoce el nick real. CharacterCreate no se reenvía (recrearía el personaje en pantalla), así
        // que a quien YA lo tenía en pantalla se le corrige el nombre con UpdateTagAndStatus (mismo
        // paquete que GuildManager.RefreshCharStatus usa para actualizar el tag de clan sin relogueo).
        // "#"+nick, no NombreBotProtocolo: el cliente sólo saca UN "#" en este paquete (no reaplica la
        // normalización "#*"→"#" que sí hace CHARACTER_CREATE para BotSmart, ver incoming.js).
        string tagNick = "#" + nick;
        for (int i = 1; i <= UserListManager.LastUser; i++)
        {
            var o = UserListManager.UserList[i];
            if (o != null && o.flags.UserLogged && o.Conn != null && o.Pos.Map == bot.Map)
                ServerPackets.UpdateTagAndStatus(o.Conn, bot.CharIndex, tagNick, bot.Status, 0);
        }
    }
    ...
    public static void ProvocarNpc(NpcInstance npc, User atacante)
    {
        ...
        // [[aggro pegajoso]] Bug reportado: el NPC que YA estaba peleando contra un jugador soltaba
        // ese objetivo en cuanto otro usuario le pegaba, porque acá el target se seteaba siempre al
        // último atacante. Con eso, un tercero te "levantaba" el bicho de encima con un solo golpe.
        // Ahora el atacante nuevo pasa a ser el objetivo SOLO si el anterior ya no se puede
        // perseguir (murió, deslogueó, cambió de mapa, se ocultó o se fue de rango) — lo decide
        // SigueTrabadoConSuObjetivo, con el mismo criterio que la IA usa para soltar el target.
        // El AttackedBy/AttackedFirstBy de arriba (dueño del loot/exp) NO cambia: es otra cosa.
        bool mantieneObjetivo = npc.TargetUser != atacante.id && SigueTrabadoConSuObjetivo(npc);
        if (!mantieneObjetivo)
        {
    ...
    /// **y ProvocarNpc decidió cambiar de objetivo**: gira a encararlo y, si ya está adyacente,
    /// pega/castea ya mismo (respeta su propio cooldown vía PuedeAtacarNpc, así que si ya había
    /// golpeado hace poco esta llamada simplemente no hace nada extra).
    ///
    /// [[aggro pegajoso]] Con el aggro pegajoso el cambio de objetivo ya NO es incondicional: si el
    /// NPC se quedó con su objetivo anterior (`npc.TargetUser != atacante.id` al volver de
    /// ProvocarNpc) acá no se hace nada — sería justo el bug que se arregló, girarse a pegarle al que
    /// le acaba de dar un golpe mientras sigue trabado con otro.
    ///
    ...
    public static void ReaccionInmediataANuevoAtacante(NpcInstance npc, User atacante, int prevTarget)
    {
        ...
        if (npc.TargetUser != atacante.id) return; // [[aggro pegajoso]] no le robó el target: sigue con el suyo
