...
public static class PacketHandler
{
    ...
    private static void EngancharEspectador(Connection conn, string nombre)
    {
        ...
        // Tour de mapas para stream: "__mapa__:N" → la cámara sigue a un NPC del mapa N
        // (el cliente con ?pj=__tour__ va mandando uno por salto, ver Espia.EmpezarEspectadorMapa).
        if (nombre.Trim().StartsWith(Game.Espia.TOUR_PREFIJO, StringComparison.OrdinalIgnoreCase)
            && int.TryParse(nombre.Trim().Substring(Game.Espia.TOUR_PREFIJO.Length), out int mapaTour))
        { Game.Espia.EmpezarEspectadorMapa(conn, mapaTour); return; }
