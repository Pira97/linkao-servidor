...
public static class ServerPackets
{
    ...
    /// <summary>NpcHp (221): Integer(charIndex) + Long(minHP) + Long(maxHP). Vida de un NPC para
    /// la barra que el cliente dibuja debajo de él al pegarle. Sólo a clientes con ClientCaps bit7.</summary>
    public static void NpcHp(Connection conn, short charIndex, int minHP, int maxHP)
    {
        if (conn == null || !conn.SoportaVidaNpc) return;
        var p = new ByteQueue();
        p.WriteByte((byte)ServerPacketID.NpcHp);
        p.WriteInteger(charIndex);
        p.WriteLong(minHP);
        p.WriteLong(maxHP);
        Send(conn, p);
